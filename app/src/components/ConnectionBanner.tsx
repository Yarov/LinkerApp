...
export default function ConnectionBanner() {
  const { connected, checking, retry, vpnConnected } = useMikrotik();
  const [dismissed, setDismissed] = useState(false);

  // Reset dismissed state when connection status changes
  useEffect(() => {
    if (!connected || !vpnConnected) {
      setDismissed(false);
    }
  ...
  return (
    <div className="sticky top-0 z-50 flex items-center gap-3 border-b border-[#f59e0b]/30 bg-[#fffbeb] px-5 py-2.5">
      {!connected ? (
        <WifiOff className="h-4 w-4 shrink-0 text-[#b45309]" />
      ) : (
        <Shield className="h-4 w-4 shrink-0 text-[#b45309]" />
      )}
      ...
      <button
        onClick={retry}
        disabled={checking}
        className="flex items-center gap-1.5 rounded-lg border border-[#f59e0b]/30 bg-white px-3 py-1 text-xs font-medium text-[#b45309] transition-colors hover:bg-[#fef3c7] disabled:opacity-50"
      >
        <RefreshCw
      ...
      <button
        ...
        className="rounded-lg p-1 text-[#b45309]/60 transition-colors hover:bg-[#fef3c7] hover:text-[#b45309]"
      >
